...
import { BaseText, FilterChip } from "@/components";
            ...
            >
              ...
              <View style={style.HomeFilterItemLabelContainer}>
                <FlatList
                  ...
                  data={item.options}
                  keyExtractor={(option: IOptions, optionIndex: number) =>
                    `home-filter-${index}-${item.label}-${optionIndex}-${option.value}`
                  }
                  ItemSeparatorComponent={() => (
                    <View style={style.HomeFilterItemSeparator} />
                  )}
                  contentContainerStyle={style.HomeFilterListContainer}
                  renderItem={({ item: option }: { item: IOptions }) => (
                    <FilterChip
                      label={option.label}
                      value={option.value}
                    />
